...
{                                                                        // Style pour les ZAI
    ...
    ui->listWidget->setContextMenuPolicy(Qt::CustomContextMenu);        // Ajoute un menu contextuel pour la listeWidget
...
/**
 *
 *   Permet de changer de couleur son pseudo
 *
 */
...
/**
 * Slot appele lorsque l'on clique droit sur la liste des utilisateurs
 * @param pos: la position dans l'application
 */

void Accueil::customMenuRequested(const QPoint &pos){
    QPoint globalPos= ui->listWidget->mapToGlobal(pos);                       // Recupere la position dans le widget
    ...
    QAction *selected_Item=menu->exec(globalPos);                              // Ouvre le menu contextuel au point du clique

    if(selected_Item == discussionPrivee ){
        ListClient *choixClient=new ListClient(users);
        connect(choixClient,SIGNAL(nouvelleListeClient(QStringList)),         // Recoit le signal emis par ListeClient

                this,SLOT(nouvelleListePrivee(const QStringList &)));
    }
...
/**
 * @brief Accueil::nouvelleListePrivee
 * @param liste
 */
